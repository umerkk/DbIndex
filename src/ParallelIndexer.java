...
public class ParallelIndexer implements Runnable {
	...
	public ParallelIndexer(BlockingQueue<byte[]> e,HashMap<Integer,Integer> _hash, int byteSize)
	{
		this.dataQueue = e;
		this.hashmap = _hash;
	...
	public static void insert(File filename, long offset, byte[] content) {
		try { 
			...
			RandomAccessFile rtemp = new RandomAccessFile(new File(filename.getName() + "~"), "rw");
